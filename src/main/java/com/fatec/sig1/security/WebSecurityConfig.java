...
public class WebSecurityConfig extends WebSecurityConfigurerAdapter {
    ...
    protected void configure(HttpSecurity http) throws Exception {
		...
		.authorizeRequests().antMatchers("/").permitAll() //
                .antMatchers("/profissional").hasRole("ADMIN") // somente login maria
				...
				.antMatchers("/sig/profissionais", "/sig/profissionais/**").hasAnyRole("ADMIN", "OUTROS")
				//.antMatchers("/sig/pedidos", "/sig/pedidos/**").hasRole("OUTROS")
				//.antMatchers("/sig/cliente", "/sig/cliente/**").hasRole("OUTROS")
				//.antMatchers("/sig/clientes", "/sig/clientes/**").hasRole("OUTROS")

                .antMatchers("/sig/pedido", "/sig/pedido/**").hasAnyRole("ADMIN", "OUTROS")
				.antMatchers("/sig/pedidos", "/sig/pedidos/**").hasAnyRole("ADMIN", "OUTROS")
                .antMatchers("/sig/produto", "/sig/produto/**").hasAnyRole("ADMIN", "VEND")
				.antMatchers("/sig/produtos", "/sig/produtos/**").hasAnyRole("ADMIN", "VEND")
                .antMatchers("/sig/servico", "/sig/servico/**").hasAnyRole("ADMIN", "OUTROS")
